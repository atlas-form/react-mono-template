...
- 基础 UI 依赖：`@atlas-art/ui-react`（配套 `ui-core` 能力）
...
- 新增基础组件统一放在 `src/components/ui`：
  - React 层组件放 `src/components/ui/react`
  - Core 层能力或适配实现放 `src/components/ui/core`
...
- “布局容器” -> `src/layouts`
- “通用组件” -> `src/components`
- “新增基础组件（可归档到 UI 库）” -> `src/components/ui/react` 或 `src/components/ui/core`

...
- `src/components/`
  ...
  - `ui/`：面向未来归档到外部 UI 库的基础组件研发区。
  - `ui/react/`：React 层基础组件（优先对齐 `@atlas-art/ui-react`）。
  - `ui/core/`：Core 层能力、样式基元或适配实现（对齐 `ui-core`）。
...
- 新增通用业务组件：放 `src/components`。
- 新增基础组件：优先复用 `@atlas-art/ui-react`；若依赖库暂未提供，则先参考 `ui-react` + `ui-core` 后在 `src/components/ui/react` 或 `src/components/ui/core` 开发。
